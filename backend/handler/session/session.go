...
import (
	...
	"wsai/backend/application/session"
	"wsai/backend/model"
...
// @Failure      200   {string}  string   "SSE 错误事件"
...
func CreateStreamSessionAndSendFirstMessage(c *gin.Context) {
	...
	if len(req.DocumentIDs) > 0 {
		if err := ragapp.BindDocumentsToSession(userName, sessionID, req.DocumentIDs); err != nil {
			c.SSEvent("error", gin.H{"message": "failed to bind documents"})
			return
		}
	}
...
func GetMessageHistory(c *gin.Context) {
	...
	if code_ != code.CodeSuccess {
		res := new(GetMeaasgeHistoryResponse)
		c.JSON(http.StatusOK, res.CodeOf(code_)) // 保持统一风格：HTTP 200 加业务错误码。
		return
	}
